...
class MultiLabelConfusionMatrix(Metric):
    ...
    Arguments:
        num_classes: `int`, the number of labels/classes the prediction task can
                     have.
    ...
    Standalone usage:
        ...
        >>> m = MultiLabelConfusionMatrix(num_classes=2, result_type='overall')
        ...
        >>> y_true = np.array([[1, 0, 1], [0, 1, 0]], dtype=np.int32)
        >>> y_pred = np.array([[1, 0, 0], [0, 1, 1]], dtype=np.int32)
        >>> m = MultiLabelConfusionMatrix(num_classes=2, result_type='class-wise')
        >>> m.update_state(y_true, y_pred)
        >>> m.result().numpy()
        array([[[1., 0.],
        [0., 1.]],
    ...
    def __init__(self, num_classes, result_type='overall',
                 name='confusion_matrix', dtype=None, **kwargs):
        super(MultiLabelConfusionMatrix,
              self).__init__(name=name, dtype=dtype, **kwargs)
        # Validate arguments
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be an "
                             "integer value. ")

        if not result_type.lower() in {'overall','class', 'class-wise'}:
            raise ValueError("The `result_type` argument should be either "
                             "'overall' or 'class-wise'. ")
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if sample_weight is not None:
            raise ValueError("`sample_weight` is not None. "
                             "Be aware that the MultiLabelConfusionMatrix "
                             "metric does not take `sample_weight` "
                             "into account when computing its value.")

        # Compute the confusion matrix values
        true_positive = [tf.reduce_sum(tf.cast(tf.logical_and(
            tf.equal(y_true, class_id),
            tf.equal(y_pred, class_id)), self._dtype))
                         for class_id in range(self.num_classes)]
        true_negative = [tf.reduce_sum(tf.cast(tf.logical_and(
            tf.not_equal(y_true, class_id),
            tf.not_equal(y_pred, class_id)), self._dtype))
                         for class_id in range(self.num_classes)]
        false_positive = [tf.reduce_sum(tf.cast(tf.logical_and(
            tf.not_equal(y_true, class_id),
            tf.equal(y_pred, class_id)), self._dtype))
                          for class_id in range(self.num_classes)]
        false_negative = [tf.reduce_sum(tf.cast(tf.logical_and(
            tf.equal(y_true, class_id),
            tf.not_equal(y_pred, class_id)), self._dtype))
                          for class_id in range(self.num_classes)]
    ...
    def result(self):
        """
        Returns the confusion matrix values based on the specified result_type
        argument.
        """
        ...
        confusion_matrix = tf.reshape(tf.transpose(
            flat_confusion_matrix), [-1, 2, 2])
...
class IoU(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='iou', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be "
                             "an integer.")
        if not result_type.lower() in {'overall','mean', 'class', 'class-wise'}:
            raise ValueError("The `result_type` argument should be either "
                             "'overall', 'mean', or 'class-wise'.")

        # Import the confusion matrix attributes
        self.num_classes = num_classes
        super().__init__(self.num_classes, result_type='class',
    ...
    def result(self):
        """
        Computes the IoU from the true positive, false postive, and false
        negative values from the confusion matrix computed by the
        update_state funtion.
        """
...
class Precision(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='precision', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be "
                             "an integer.")
...
class Recall(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='recall', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be "
                             "an integer.")

        if not result_type.lower() in {'overall', 'mean',
                                       'class', 'class-wise'}:
            raise ValueError("The `result_type` argument should be "
                             "either 'overall', 'mean', or 'class-wise'.")
...
class Specificity(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='specificity', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be "
                             "an integer.")
    ...
    def result(self):
        """
        Computes the specificity metric from the true negatives and false
        positives values from the confusion matrix computed by the
        update_state funtion.
        """
...
class Sensitivity(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='sensitivity', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
    ...
    def result(self):
        """
        Computes the sensitivity metric from the true positives and true
        negatives values from the confusion matrix computed by the
        update_state funtion.
        """
...
class TDR(MultiLabelConfusionMatrix):
    """
...
class F1Score(MultiLabelConfusionMatrix):
    """
    ...
    def __init__(self, num_classes, result_type='mean',
                 name='f1_score', dtype=None, **kwargs):
        # Validate class arguments/attributes
        if not isinstance(num_classes, int):
            raise ValueError("The `num_classes` argument should be "
                             "an integer.")
    ...
    def result(self):
        ...
        if self.result_type in {'class', 'class-wise'}:
            ...
            recall = tf.math.divide_no_nan(tf.reduce_sum(
                self.true_positives),
                tf.reduce_sum((self.true_positives +
                               self.false_negatives)))
            recall = tf.cast(recall, self._dtype)
...
def classification_report(num_classes,
                          y_true,
                          y_pred,
                          metrics=["TP", "TN", "FP", "FN", "Recall", "Precision",
                                   "Specificity", "Sensitivity", "IoU", "TDR",
                                   "F1Score"],
                          target_names=None,
                          report_type='table',
    ...
    Arguments:
        ...
        metrics: a list containing one or more of the following metrics:
                 'TP', 'TN', 'FP', 'FN', 'Recall', 'Precision', 'Specificity',
                 'Sensitivity', 'F1 Score', 'IoU', 'TDR'. Default list displays
                 all of the aforementioned metrics.
        target_names: (optional) a list or tuple containing names of the
                      segmentation classes.
        report_type: (optional) one of 'dictionary', 'table' or 'array'. Default
                     is table.
        table_type: (optional) one of 'pandas-dataframe' (to display output as a
                             pandas dataframe), 'or tabulate-table (to display
                             output as a tabulate table).  Default is
                             pandas-dataframe.
        show_mean_evaluations: (optional) one of True (to display both mean
                             evaluations), 'or False (to not display both mean
                             evaluations). Default is True.
        show_classwise_evaluations: (optional) one of True (to display class-wise
                              evaluations), 'or False (to display only the overall
                              evaluations). Default to True.
        decimals: (optional) integer value. Number of decimal places to round to.
                             Default to 2.

    Returns:
        A pandas dataframe or tabulate table containing output of the
    ...
    if target_names == None:
        target_names = ["Class " + str(class_id+1)
                        for class_id in range(num_classes)]
    else:
        if not(isinstance(target_names, list) or
               isinstance(target_names, tuple)):
            raise ValueError("The `target_names` argument should be None "
                             "(display default class names) "
                             "or a list/tuple containing the class names.")
        elif not all(isinstance(name, str) for name in target_names):
            raise ValueError("The `target_names` argument list items "
                             "should be string.")
        elif len(target_names) != num_classes:
            raise ValueError("The numer of class names specified in the "
                             "`target_names` argument list/tuple should "
                             "match with the `num_classes` argument value.")

    # 5. report_type='table'
    if not report_type in {'array','dictionary','table'}:
        raise ValueError("The `report_type` argument should be one of "
                         "'array', 'dictionary', or 'table'.")

    # 6. table type
    if not table_type in {'pandas-dataframe', 'tabulate-table'}:
        raise ValueError("The `table_type` argument can only be either "
                         "'pandas-dataframe' or 'tabulate-table'.")

    # 7. show_mean_evaluations
    if not show_mean_evaluations in {True, False}:
        raise ValueError("The `show_mean_evaluations` argument can only "
                         "be either True or False.")

    # 8. show_classwise_evaluations
    if not show_classwise_evaluations in {True, False}:
        raise ValueError("The `show_classwise_evaluations` argument can "
    ...
    # -------------------------------------------------------------------------#
    # Build classification report model
    # -------------------------------------------------------------------------#
    evaluations = {}
    ...
    for result_type in result_types:
        if result_type == 'mean':
            ...
            matrix = MultiLabelConfusionMatrix(num_classes=num_classes,
        ...
        recall = Recall(num_classes=num_classes,
                        result_type=result_type, dtype=tf.float32)
        ...
        # Sensitivity
        sensitivity = Sensitivity(num_classes=num_classes,
                                  result_type=result_type, dtype=tf.float32)
        ...
        # TDR
        tdr = TDR(num_classes=num_classes,
                  result_type=result_type, dtype=tf.float32)
        ...
        evaluations[result_type] = {"TP": true_positives,
                                    "TN": true_negatives,
                                    "FP": false_positives,
                                    "FN": false_negatives,
                                    "Recall": recalls,
    ...
    if report_type == 'dictionary':
        ...
        overall_data = ["Overall"] + [
            evaluations['overall'][metrics[metric_id]][0]
            if isinstance(evaluations['overall'][metrics[metric_id]], np.ndarray)
            else evaluations['overall'][metrics[metric_id]]
            for metric_id in range(len(metrics))]
        mean_data = ["Mean"] + [
            evaluations['mean'][metrics[metric_id]]
            if not isinstance(evaluations['mean'][metrics[metric_id]], np.ndarray)
            else evaluations['mean'][metrics[metric_id]][0] for metric_id in
            range(len(metrics))]
        classwise_data = [target_names] + [
            evaluations['class-wise'][metrics[metric_id]] for metric_id in
            range(len(metrics))]
        ...
        if report_type=='array':
            ...
            if show_mean_evaluations:
                report_data = np.concatenate((report_data, mean_data),
                                             axis=0)
            if show_classwise_evaluations:
                report_data = np.concatenate((report_data, classwise_data),
                                             axis=0)
            ...
            if table_type == 'pandas-dataframe':
                ...
                report = print(tabulate(report_data,
...
################################################################################
def plot_confusion_matrix(num_classes, y_true, y_pred, target_names=None):
    """
    Plots the confusion matrix for the
    ...
    cm_array = cm.result().numpy()

    pred_names = np.array(target_names)
    true_names = np.array([-] + target_names).reshape

    matrix = np.concatenate((report, mean_data), axis=0)
    matrix =
